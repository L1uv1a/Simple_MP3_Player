...
Thread LCD_thread ; //Displays welcome message on LCD 
...
Thread Tune_thread;         //Reads the select buttons and readies the song 
    ...
    { 
        ...
        {        //"primed" is set by Interrupt 
            ...
            {     //read song selection and load song pointer 
                ...
                case 0x07:song_ptr=&Los_Santos;break; 
...
Thread Play_thread;        //plays the chosen tune 
...
{ 
    ...
    { 
        ...
        { 
            ...
            thread_sleep_for(750*(*song_ptr).beat[i]); 
...
int main() { 
    ...
    LCD_thread.start(callback(LCD_cont,&playing_led)); 

    Tune_thread.start(callback(Tune_select,&choose)); 

    Play_thread.start(callback(Play_tune,&play)); 
